...
HuePlatform.prototype.initialize = function() {
  this._connect((success) => {

  });
};

HuePlatform.prototype._connect = function(callback) {
  this._authenticateClients((success) => {
    for (let clientId in this.clients) {
      let client = this.clients[clientId];
      var uuid = UUIDGen.generate(client.bridgeId);

      var bridgeAccessory = new Accessory(client.bridgeName, uuid);
    ...
    .then((bridges) => {
      if (bridges.length > 0) {
        for (let bridge of bridges) {
          ...
          let client = new HueClient(bridge.id, bridge.ip);
