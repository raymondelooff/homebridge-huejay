...
class HuePlatform {
      ...
      client.sensors.getAll()
        .then(sensors => {
          for (const sensor of sensors) {
            ...
            const accessory = this._createPlatformAccessory(client, sensor);
  ...
  _getCachedPlatformAccessory(accessory) {
    for (const cachedAccessory of this.cachedAccessories) {
      if (cachedAccessory.context.uniqueId === accessory.uniqueId) {
  ...
  _createPlatformAccessory(client, hueAccessory) {
    let accessory = this._getCachedPlatformAccessory(hueAccessory);
    ...
    if (accessory === null) {
      accessory = this.accessoryFactory.createAccessory(client, hueAccessory);
